...
const Journal: React.FC<JournalProps> = ({ 
  ...
  const [activeFormTab, setActiveFormTab] = useState<'details' | 'review'>('details');
  ...
  }, [isModalOpen, modalStep, activeFormTab, language]);
  ...
  const saveTradeToState = () => {
        ...
        handleCloseModal(); setFormData(initialFormState); setEditingTradeId(null); setToastMessage(isOpenTrade ? "Trade saved as OPEN position!" : t.journal.saveSuccess); setShowToast(true); setTimeout(() => setShowToast(false), 3000);
  }
    ...
    <div className="space-y-6">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm animate-fade-in">
            <div className={`bg-white dark:bg-slate-900 rounded-2xl w-full shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col ${modalStep === 'checklist' && !editingTradeId ? 'max-w-md max-h-[90vh]' : 'max-w-4xl h-auto'}`}>
                {modalStep === 'checklist' && !editingTradeId ? (
                    ...
                    <div className="flex flex-col h-full bg-white dark:bg-slate-900 rounded-2xl overflow-hidden">
                        <div className="flex items-center justify-between px-6 pt-5 pb-0"><div className="flex items-center gap-2"><Plus className="w-5 h-5 text-indigo-600 dark:text-indigo-400" /><div><h3 className="text-xl font-bold text-slate-900 dark:text-white leading-none">{t.journal.modalTitle}</h3><p className="text-xs text-slate-400 mt-1">{t.journal.modalSubtitle}</p></div></div><div className="flex items-center gap-4"><div className="bg-slate-100 dark:bg-slate-800 p-1 rounded-lg flex text-xs font-bold"><button onClick={() => setActiveFormTab('details')} className={`px-4 py-1.5 rounded-md transition-all ${activeFormTab === 'details' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>{t.journal.detailsTab}</button><button onClick={() => setActiveFormTab('review')} className={`px-4 py-1.5 rounded-md transition-all ${activeFormTab === 'review' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>{t.journal.reviewTab}</button></div><button onClick={handleCloseModal} className="text-slate-400 hover:text-slate-600 dark:hover:text-white"><X className="w-6 h-6" /></button></div></div>
                        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">{activeFormTab === 'details' ? (<><div className="flex flex-col md:flex-row gap-6"><div className="flex-1"><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">{t.journal.asset}</label><div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5" /><input type="text" required list="symbol-options" value={formData.symbol} onChange={e => setFormData({...formData, symbol: e.target.value.toUpperCase()})} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl pl-12 pr-4 h-14 text-2xl font-bold text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 transition-all placeholder:text-slate-300" placeholder={language === 'cn' ? '例如 BTC' : 'BTC'} /><datalist id="symbol-options">{uniqueSymbols.map(s => <option key={s} value={s} />)}</datalist></div></div><div className="flex-1"><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">{t.journal.directionLabel}</label><div className="grid grid-cols-2 gap-0 bg-slate-100 dark:bg-slate-800 rounded-2xl p-1 h-14"><button type="button" onClick={() => setFormData({...formData, direction: Direction.LONG})} className={`rounded-xl font-bold text-lg transition-all flex items-center justify-center gap-2 ${formData.direction === Direction.LONG ? 'bg-emerald-500 text-white shadow-md' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}><TrendingUp className="w-5 h-5" /> {t.journal.long}</button><button type="button" onClick={() => setFormData({...formData, direction: Direction.SHORT})} className={`rounded-xl font-bold text-lg transition-all flex items-center justify-center gap-2 ${formData.direction === Direction.SHORT ? 'bg-rose-500 text-white shadow-md' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}><TrendingDown className="w-5 h-5" /> {t.journal.short}</button></div></div></div><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div className="relative"><div className="absolute -top-2 left-3 bg-white dark:bg-slate-900 px-1 text-[10px] font-bold text-indigo-500 uppercase tracking-wider z-10">{t.journal.openTimeLabel}</div><div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3 flex items-center"><Calendar className="w-4 h-4 text-indigo-400 mr-3" /><input ref={entryDateInputRef} type="text" required value={formData.entryDate} onChange={e => setFormData({...formData, entryDate: e.target.value})} className="w-full bg-transparent border-none text-sm font-medium text-slate-900 dark:text-white outline-none" placeholder="Select Date & Time" /><Clock className="w-4 h-4 text-indigo-400 ml-auto" /></div></div><div className="relative"><div className="absolute -top-2 left-3 bg-white dark:bg-slate-900 px-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider z-10">{t.journal.closeTimeLabel}</div><div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3 flex items-center"><Calendar className="w-4 h-4 text-slate-400 mr-3" /><input ref={exitDateInputRef} type="text" value={formData.exitDate} onChange={e => setFormData({...formData, exitDate: e.target.value})} className="w-full bg-transparent border-none text-sm font-medium text-slate-900 dark:text-white outline-none placeholder:text-slate-300" placeholder={language === 'cn' ? '可选' : 'Optional'} /><button type="button" onClick={() => handleSetTimeNow('exitDate')} className="hover:text-indigo-500 ml-auto text-slate-400"><Clock className="w-4 h-4" /></button></div></div></div><div><div className="text-xs font-bold text-slate-400 uppercase flex items-center gap-2 mb-3"><Zap className="w-3 h-3" /> {t.journal.executionData}</div><div className="grid grid-cols-3 gap-4"><div><label className="text-[10px] text-slate-400 font-bold ml-1 mb-1 block">{t.journal.entryLabel}</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-xs">$</span><input type="number" step="any" required placeholder={t.journal.entryPrice} value={formData.entryPrice ?? ''} onChange={e => setFormData({...formData, entryPrice: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl pl-6 pr-3 py-3 text-sm font-medium outline-none focus:border-indigo-500 text-slate-900 dark:text-white" /></div></div><div><label className="text-[10px] text-slate-400 font-bold ml-1 mb-1 block">{t.journal.exitLabel}</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-xs">$</span><input type="number" step="any" placeholder={t.journal.exitPrice} value={formData.exitPrice ?? ''} onChange={e => setFormData({...formData, exitPrice: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl pl-6 pr-3 py-3 text-sm font-medium outline-none focus:border-indigo-500 text-slate-900 dark:text-white" /></div></div><div><label className="text-[10px] text-slate-400 font-bold ml-1 mb-1 block">{t.journal.sizeLabel}</label><input type="number" step="any" required placeholder={language === 'cn' ? '仓位数量' : 'Qty'} value={formData.quantity ?? ''} onChange={e => setFormData({...formData, quantity: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-3 text-sm font-medium outline-none focus:border-indigo-500 text-slate-900 dark:text-white" /></div></div></div><div><div className="text-xs font-bold text-slate-400 uppercase flex items-center gap-2 mb-3"><ShieldCheck className="w-3 h-3" /> {t.journal.riskManagement}</div><div className="grid grid-cols-3 gap-4"><div><label className="text-[10px] text-slate-400 font-bold ml-1 mb-1 block">{t.journal.leverageLabel}</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-xs">#</span><input type="number" placeholder="1" value={formData.leverage ?? ''} onChange={e => setFormData({...formData, leverage: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl pl-6 pr-3 py-3 text-sm font-medium outline-none focus:border-indigo-500 text-slate-900 dark:text-white" /></div></div><div><label className="text-[10px] text-amber-500 font-bold ml-1 mb-1 block flex items-center gap-1">{language === 'cn' ? '风险金额 ($)' : t.journal.riskAmountLabel} <InfoTooltip text="Stop Loss Amount" /></label><div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-amber-500"><AlertTriangle className="w-3 h-3" /></span><input type="number" placeholder={language === 'cn' ? '风险金额 ($)' : 'Risk $'} value={formData.riskAmount ?? ''} onChange={e => setFormData({...formData, riskAmount: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-800 border border-amber-200 dark:border-amber-900/50 rounded-xl pl-8 pr-10 py-3 text-sm font-medium outline-none focus:border-amber-500 placeholder:text-amber-300 text-slate-900 dark:text-white" /></div></div><div><label className="text-[10px] text-slate-400 font-bold ml-1 mb-1 block">{language === 'cn' ? '手续费' : t.journal.fees}</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-xs">%</span><input type="number" step="any" placeholder={language === 'cn' ? '手续费' : '% Fees'} value={formData.fees ?? ''} onChange={e => setFormData({...formData, fees: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl pl-6 pr-3 py-3 text-sm font-medium outline-none focus:border-indigo-500 text-slate-900 dark:text-white" /></div></div></div></div><div className="flex gap-6 h-40"><div className="flex-1 bg-sky-50/50 dark:bg-slate-800/50 rounded-2xl p-6 border border-sky-100 dark:border-slate-700 flex flex-col justify-between relative group"><p className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t.journal.netPnlLabel}</p><div className="flex items-baseline gap-1 absolute bottom-6 left-6 right-6"><input type="number" step="any" value={formData.pnl ?? ''} onChange={e => setFormData({...formData, pnl: e.target.value})} placeholder="0.00" className={`w-full bg-transparent border-none outline-none text-5xl font-black font-mono tracking-tighter placeholder:text-slate-200 dark:placeholder:text-slate-700 pr-14 ${Number(formData.pnl) > 0 ? 'text-emerald-500' : Number(formData.pnl) < 0 ? 'text-rose-500' : 'text-slate-900 dark:text-white'}`} /></div><span className="text-slate-400 text-xl font-bold absolute bottom-8 right-6 pointer-events-none">$</span></div><div className="w-5/12 flex flex-col gap-3"><div className="relative h-12"><Briefcase className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" /><select value={formData.setup} onChange={e => setFormData({...formData, setup: e.target.value})} className="w-full h-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl pl-10 pr-8 text-sm outline-none focus:border-indigo-500 appearance-none text-slate-900 dark:text-white cursor-pointer"><option value="" disabled>{t.journal.selectStrategy}</option>{strategies.map(s => (<option key={s.id} value={s.name}>{s.name}</option>))}</select><div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none"><ChevronDown className="w-4 h-4 text-slate-400" /></div></div><label className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors group">{formData.images && formData.images.length > 0 ? (<div className="text-xs text-indigo-500 font-bold">{formData.images.length} Image(s)</div>) : (<><div className="flex flex-col items-center justify-center"><Upload className="w-5 h-5 text-slate-400 group-hover:text-indigo-500 mb-1" /><p className="text-[10px] text-slate-400 font-bold uppercase">IMG</p><p className="text-[9px] text-slate-400 group-hover:text-indigo-400 transition-colors font-medium mt-1">{language === 'cn' ? '支持 Ctrl+V 粘贴' : 'Supports Ctrl+V Paste'}</p></div><input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} /></>)}</label></div></div></>) : (<div className="space-y-6 animate-fade-in"><div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700"><label className="block text-xs font-bold text-slate-500 uppercase mb-2 flex items-center gap-2"><FileText className="w-4 h-4" /> {t.journal.notes}</label><textarea value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} className="w-full h-32 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-sm outline-none focus:border-indigo-500 resize-none transition-all placeholder:text-slate-400 text-slate-900 dark:text-white" placeholder="Execution notes, pre-trade thoughts..."></textarea></div><div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700"><label className="block text-xs font-bold text-slate-500 uppercase mb-2 flex items-center gap-2"><BookCheck className="w-4 h-4" /> {t.journal.reviewNotes}</label><textarea value={formData.reviewNotes} onChange={e => setFormData({...formData, reviewNotes: e.target.value})} className="w-full h-32 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-sm outline-none focus:border-indigo-500 resize-none transition-all placeholder:text-slate-400 text-slate-900 dark:text-white" placeholder="Post-trade analysis, lessons learned..."></textarea></div><div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4"><label className="block text-xs font-bold text-slate-500 uppercase mb-3">{t.journal.mistakes}</label><div className="flex flex-wrap gap-2 mb-3">{formData.mistakes?.map((m: any, i: number) => (<span key={i} className="bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 px-2 py-1 rounded text-xs border border-rose-100 dark:border-rose-800 flex items-center gap-1">{m}<button type="button" onClick={() => removeMistake(i)}><X className="w-3 h-3" /></button></span>)) }</div><div className="flex gap-2"><input type="text" value={newMistake} onChange={e => setNewMistake(e.target.value)} onKeyDown={handleAddMistake} placeholder={t.journal.mistakesPlaceholder} className="flex-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm outline-none focus:border-indigo-500 text-slate-900 dark:text-white" /><button type="button" onClick={() => handleAddMistake({ key: 'Enter', preventDefault: () => {} } as any)} className="p-2 bg-slate-100 dark:bg-slate-800 rounded-lg text-slate-600 dark:text-slate-400 hover:bg-rose-50 hover:text-rose-500 transition-colors"><Plus className="w-5 h-5" /></button></div></div></div>)}</form>
