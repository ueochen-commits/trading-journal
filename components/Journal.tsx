...
const Journal: React.FC<JournalProps> = ({ 
  ...
  const saveTradeToState = () => {
        ...
        if (Object.keys(errors).length > 0) {
            ...
            setToastType('error');
            setToastMessage(language === 'cn' ? '请填写必填项：交易对、开仓时间、开仓价格' : 'Please fill required fields: Symbol, Entry Time, Entry Price');
            setShowToast(true);
            setTimeout(() => setShowToast(false), 3000);
            return;
        }
