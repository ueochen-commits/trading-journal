      ...
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 560, maxHeight: '88vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.18)' }}>
        ...
        <div style={{ padding: '20px 24px 14px', borderBottom: '1px solid #f0f0f6', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: '#1a1d2e', margin: 0 }}>{language === 'cn' ? '规则设置' : 'Rules'}</h2>
          ...
          <p style={{ fontSize: 12, color: '#9396aa', marginTop: 4, marginBottom: 0 }}>{language === 'cn' ? '修改只会影响今天及以后的得分记录。' : 'Changes you make will only update your scoring for today and for future days.'}</p>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          <div style={{ padding: '16px 24px 20px', borderTop: '1px solid #f0f0f6' }}>
            ...
            {!showResetConfirm ? (
              <button onClick={() => setShowResetConfirm(true)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 18px', borderRadius: 8, border: 'none', background: '#ff4d6a', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>🔄 {language === 'cn' ? '重置所有进度' : 'Reset all progress'}</button>
            ) : (
        ...
        <div style={{ padding: '14px 24px', borderTop: '1px solid #f0f0f6', display: 'flex', justifyContent: 'flex-end', gap: 10, flexShrink: 0 }}>
          <button onClick={onClose} style={{ padding: '9px 22px', borderRadius: 8, border: '1px solid #e0e0ea', background: 'transparent', fontSize: 13, color: '#4a4d6a', cursor: 'pointer', fontWeight: 500 }}>{language === 'cn' ? '取消' : 'Cancel'}</button>
          <button onClick={() => { onSave(settings, manualRules); setIsDirty(false); onClose(); }} disabled={!isDirty} style={{ padding: '9px 22px', borderRadius: 8, border: 'none', fontSize: 13, fontWeight: 600, cursor: isDirty ? 'pointer' : 'not-allowed', background: isDirty ? '#6366f1' : '#e8e8f0', color: isDirty ? '#fff' : '#b0b3c6' }}>{language === 'cn' ? '保存' : 'Save changes'}</button>
        </div>
      </div>
    </div>
  );
