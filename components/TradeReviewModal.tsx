...
const TagSelector: React.FC<TagSelectorProps> = ({ 
    ...
    useEffect(() => {
        if (!isMenuOpen) {
            setCategoryDraftName(cat.label);
            setCategoryDraftColor(cat.color);
        }
    }, [isMenuOpen, cat.label, cat.color]);
    ...
    const handleSaveCategoryMeta = () => {
        ...
        if (!nextName) return;
        if (nextName !== cat.label) {
            onRenameCategory(nextName);
        }
        if (categoryDraftColor !== cat.color) {
            onChangeColor(categoryDraftColor);
        }
        setIsMenuOpen(false);
    };
        ...
        >
            ...
            <div className="flex items-center justify-between mb-2">
                ...
                <div className="relative" ref={menuRef}>
                    ...
                    {isMenuOpen && (
                        <div className="absolute right-0 top-7 z-50 w-[214px] rounded-[16px] border border-slate-200 bg-white p-3 shadow-[0_18px_40px_rgba(15,23,42,0.16)] animate-fade-in-up dark:border-slate-700 dark:bg-slate-900">
                            <div className="mb-3 flex items-center gap-2 text-[13px] font-semibold text-slate-700 dark:text-slate-200">
                                ...
                                <span>{language === 'cn' ? '重命名' : 'Rename'}</span>
                            </div>

                            <div className="mb-3">
                                <div className="mb-2 text-[11px] font-medium text-slate-400 dark:text-slate-500">
                                    {language === 'cn' ? '颜色' : 'Color'}
                                </div>
                                <div className="flex flex-wrap gap-2">
                                    {TAG_CATEGORY_COLORS.map(color => (
                                        <button
                                            ...
                                            style={{
                                                background: color,
                                                borderColor: categoryDraftColor === color ? '#ffffff' : 'transparent',
                                                boxShadow: categoryDraftColor === color ? '0 0 0 1.5px #0f172a' : '0 0 0 1px rgba(148,163,184,0.26)',
                                            }}
                                        />
                                    ))}
                                </div>
                            ...
                            <input
                                ...
                                onKeyDown={(e) => {
                                    if (e.key === 'Enter') {
                                        e.preventDefault();
                                        handleSaveCategoryMeta();
                                    }
                                    if (e.key === 'Escape') {
                                        setIsMenuOpen(false);
                                    }
                                }}
                                className="mb-3 w-full rounded-[11px] border border-slate-200 bg-white px-3 py-2 text-[13px] text-slate-700 outline-none focus:border-indigo-300 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"
                            />
