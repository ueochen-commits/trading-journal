...
const LargeOption: React.FC<LargeOptionProps> = ({ icon, title, desc, badge, onClick }) => (
    <button
        ...
        style={{
            ...
            border: '1px solid #e8e8f0', background: '#f9f9fb',
    ...
    >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 3 }}>
                ...
                {badge && (
                    <span style={{
                        fontSize: 10, fontWeight: 600, color: '#9396aa',
                        background: '#f0f0f6', border: '1px solid #e0e0ea',
                        padding: '1px 6px', borderRadius: 3, letterSpacing: '0.03em', flexShrink: 0,
                    }}>{badge.text}</span>
                )}
            </div>
            <div style={{ fontSize: 12, color: '#9396aa', lineHeight: 1.5 }}>{desc}</div>
        </div>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#d0d3e0" strokeWidth="2" strokeLinecap="round" style={{ flexShrink: 0 }}>
...
const SmallOption: React.FC<SmallOptionProps> = ({ icon, title, desc, badge, onClick }) => (
    ...
    >
        ...
        <div>
            <div style={{ fontSize: 12.5, fontWeight: 600, color: '#1a1d2e', marginBottom: 3 }}>
                {title}
                {badge && (
                    <span style={{
                        fontSize: 10, fontWeight: 600, color: '#9396aa',
                        background: '#f0f0f6', border: '1px solid #e0e0ea',
                        padding: '1px 5px', borderRadius: 3, marginLeft: 5, letterSpacing: '0.03em',
                    }}>{badge.text}</span>
                )}
            </div>
                    ...
                    <div style={{ padding: '52px 20px 8px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 10 }}>
                                <LargeOption
                                    ...
                                    onClick={() => { onClose(); onNavigate('connect-exchange'); }}
                                />
                                <LargeOption
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
                                <SmallOption
                                    icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="#6366f1"><path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm13.36-1.814a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z" clipRule="evenodd"/></svg>}
                                    title="设置心态风控"
                                    desc="制定交易纪律，建立检查清单"
                                    onClick={() => { onClose(); onNavigate('psychology'); }}
                                />
                                <SmallOption
                                    icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="#6366f1"><path d="M4.913 2.658c2.075-.27 4.19-.408 6.337-.408 2.147 0 4.262.139 6.337.408 1.922.25 3.291 1.861 3.405 3.727a4.403 4.403 0 00-1.032-.211 50.89 50.89 0 00-8.42 0c-2.358.196-4.04 2.19-4.04 4.434v4.286a4.47 4.47 0 002.433 3.984L7.28 21.53A.75.75 0 016 21v-4.03a48.527 48.527 0 01-1.087-.128C2.905 16.58 1.5 14.833 1.5 12.862V6.638c0-1.97 1.405-3.718 3.413-3.979z"/><path d="M15.75 7.5c-1.376 0-2.739.057-4.086.169C10.124 7.797 9 9.103 9 10.609v4.285c0 1.507 1.128 2.814 2.67 2.94 1.243.102 2.5.157 3.768.165l2.782 2.781a.75.75 0 001.28-.53v-2.39l.33-.026c1.542-.125 2.67-1.433 2.67-2.94v-4.286c0-1.505-1.125-2.811-2.664-2.94A49.392 49.392 0 0015.75 7.5z"/></svg>}
                                    title="加入我们"
                                    desc="扫码加入微信群交流"
                        ...
                        {/* Bottom buttons */}
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 22px 24px' }}>
                            <button
                                onClick={onClose}
                                style={{
